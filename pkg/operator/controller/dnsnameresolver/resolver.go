...
func (res *Resolver) Start() {
	...
	go func() {
		...
		for {
			...
			// Get the DNS name whose TTL will expire first. Based on the nextLookupTime for the nextDNSName, the timeTillNextLookup
...
func (res *Resolver) Add(dnsName string, resolvedAddresses []networkv1alpha1.DNSNameResolverResolvedAddress, matchesRegular bool, objName string) {
	res.dnsLock.Lock()
	...
	for _, recordType := range []uint16{dns.TypeA, dns.TypeAAAA} {
		for _, coreDNSPodIP := range coreDNSPodIPs {
			...
			if _, _, err := dnsClient.Exchange(dnsMsg, serverStr); err != nil {
				logrus.Warningf("Failed to lookup DNS name: %s from CoreDNS pod with IP: %s, err: %s", dnsName, coreDNSPodIP, err)
			}
		}
	}
	return nil
