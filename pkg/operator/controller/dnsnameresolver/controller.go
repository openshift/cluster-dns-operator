...
func NewUnmanaged(mgr manager.Manager, config Config) (controller.Controller, []cache.Cache, error) {
	...
	reconciler := &reconciler{
		...
		res:                  NewResolver(corednsEndpointsSliceCache, config.ServiceName),
	}
	...
	if err := c.Watch(source.Kind(dnsNameResolverCache, &networkv1alpha1.DNSNameResolver{}), &handler.EnqueueRequestForObject{}); err != nil {
		return nil, nil, err
	}
...
func (r *reconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	// Check if the TTLs of some of the IP addresses have expired. If so, requeue the reconcile request after the minimum remaining
	// time until the grace period gets over among that of the IP addresses with expired TTLs.
	...
	for _, resolvedName := range dnsNameResolver.Status.ResolvedNames {
		r.res.Add(string(resolvedName.DNSName), resolvedName.ResolvedAddresses, matchesRegular, request.Name)
	}
...
func removalOfIPsRequired(status *networkv1alpha1.DNSNameResolverStatus) bool {
	...
	for index, resolvedName := range status.ResolvedNames {
		updateResolvedAddresses := []networkv1alpha1.DNSNameResolverResolvedAddress{}
		for _, resolvedAddress := range resolvedName.ResolvedAddresses {
			if time.Now().Before(resolvedAddress.LastLookupTime.Time.Add(time.Second*time.Duration(resolvedAddress.TTLSeconds) + ipRemovalGracePeriod)) {
...
// TTLs is returned.
func reconcileRequired(status *networkv1alpha1.DNSNameResolverStatus) (bool, time.Duration) {
	...
	for _, resolvedName := range status.ResolvedNames {
		for _, addr := range resolvedName.ResolvedAddresses {
			if !time.Now().Before(addr.LastLookupTime.Time.Add(time.Second * time.Duration(addr.TTLSeconds))) {
				remainingTime := time.Until(addr.LastLookupTime.Time.Add(time.Second*time.Duration(addr.TTLSeconds) + ipRemovalGracePeriod))
				if !ttlExpired {
					ttlExpired = true
