...
{{range .Zones}}{{.}}:5353 {{end}}{
    ...
    ocp_dnsnameresolver {
        namespaces {{range $.DNSNameResolverNamepsaces}}{{.}}{{end}}
    }
    {{- end}}
}
{{end -}}
...
func (r *reconciler) ensureDNSConfigMap(dns *operatorv1.DNS, clusterDomain string, caBundleRevisionMap map[string]string) (bool, *corev1.ConfigMap, error) {
	...
	desired, err := desiredDNSConfigMap(dns, clusterDomain, caBundleRevisionMap, r.dnsNameResolverEnabled)
	if err != nil {
...
func desiredDNSConfigMap(dns *operatorv1.DNS, clusterDomain string, caBundleRevisionMap map[string]string, dnsNameResolverEnabled bool) (*corev1.ConfigMap, error) {
	...
	corefileParameters := struct {
		...
		DNSNameResolverNamepsaces []string
	}{
		...
		DNSNameResolverNamepsaces: []string{DefaultDNSNameResolverNamespace},
	}
