...
func (h *Handler) EnsureDefaultClusterDNS() error {
	...
	if err := sdk.Get(desired); err != nil {
		if !errors.IsNotFound(err) {
			return fmt.Errorf("failed to get clusterdns %q: %v", desired.Name, err)
		}
		if err := sdk.Create(desired); err != nil {
			return fmt.Errorf("failed to create clusterdns %q: %v", desired.Name, err)
		}
		logrus.Infof("created clusterdns %q", desired.Name)
	}
	return nil
}
	...
	for _, dns := range dnses.Items {
		...
		if dns.DeletionTimestamp != nil {
			...
			if err := h.ensureDNSDeleted(&dns); err != nil {
				errors = append(errors, fmt.Errorf("failed to delete clusterdns %q: %v", dns.Name, err))
				continue
			}
			...
			if slice.ContainsString(dns.Finalizers, ClusterDNSFinalizer) {
				...
				if err := sdk.Update(&dns); err != nil {
					errors = append(errors, fmt.Errorf("failed to remove finalizer from clusterdns %q: %v", dns.Name, err))
				}
			}
			continue
		}
		...
		if err := h.ensureCoreDNSForClusterDNS(&dns); err != nil {
			errors = append(errors, fmt.Errorf("failed to ensure clusterdns %q: %v", dns.Name, err))
		}
	}
...
func (h *Handler) ensureDNSNamespace() error {
	...
	if err := sdk.Get(ns); err != nil {
		...
		if err := sdk.Create(ns); err != nil {
			return fmt.Errorf("failed to create dns namespace %q: %v", ns.Name, err)
		}
		logrus.Infof("created dns namespace %q", ns.Name)
	}
	...
	if err := sdk.Get(sa); err != nil {
		if !errors.IsNotFound(err) {
			return fmt.Errorf("failed to get dns service account %q: %v", sa.Name, err)
		}
		if err := sdk.Create(sa); err != nil {
			return fmt.Errorf("failed to create dns service account %q: %v", sa.Name, err)
		}
		logrus.Infof("created dns service account %q", sa.Name)
	}
	...
	if err := sdk.Get(cr); err != nil {
		if !errors.IsNotFound(err) {
			return fmt.Errorf("failed to get dns cluster role %q: %v", cr.Name, err)
		}
		if err := sdk.Create(cr); err != nil {
			return fmt.Errorf("failed to create dns cluster role %q: %v", cr.Name, err)
		}
		logrus.Infof("created dns cluster role %q", cr.Name)
	}
	...
	if err := sdk.Get(crb); err != nil {
		if !errors.IsNotFound(err) {
			return fmt.Errorf("failed to get dns cluster role binding %q: %v", crb.Name, err)
		}
		if err := sdk.Create(crb); err != nil {
			return fmt.Errorf("failed to create dns cluster role binding %q: %v", crb.Name, err)
		}
		logrus.Infof("created dns cluster role binding %q", crb.Name)
	}
	return nil
}
...
func (h *Handler) ensureCoreDNSForClusterDNS(dns *dnsv1alpha1.ClusterDNS) error {
	...
	if err := sdk.Get(ds); err != nil {
		if !errors.IsNotFound(err) {
			return fmt.Errorf("failed to get dns daemonset %q: %v", ds.Name, err)
		}
		if err := sdk.Create(ds); err != nil {
			return fmt.Errorf("failed to create dns daemonset %q: %v", ds.Name, err)
		}
		logrus.Infof("created dns daemonset %q", ds.Name)
	}
		...
		case "dns":
			if h.Config.CoreDNSImage != ds.Spec.Template.Spec.Containers[i].Image {
				...
				if err := sdk.Update(ds); err != nil {
					return fmt.Errorf("failed to update dns daemonset %q: %v", ds.Name, err)
				}
				logrus.Infof("updated dns daemonset %q", ds.Name)
			}
		}
	}
	...
	if err := sdk.Get(cm); err != nil {
		if !errors.IsNotFound(err) {
			return fmt.Errorf("failed to get dns config map %q: %v", cm.Name, err)
		}
		cm.SetOwnerReferences([]metav1.OwnerReference{dsRef})
		if err := sdk.Create(cm); err != nil {
			return fmt.Errorf("failed to create dns config map %q: %v", cm.Name, err)
		}
		logrus.Infof("created dns config map %q", cm.Name)
	}
	...
	if err := sdk.Get(service); err != nil {
		if !errors.IsNotFound(err) {
			return fmt.Errorf("failed to get dns service %q: %v", service.Name, err)
		}
		service.SetOwnerReferences([]metav1.OwnerReference{dsRef})
		if err := sdk.Create(service); err != nil {
			return fmt.Errorf("failed to create dns service %q: %v", service.Name, err)
		}
		logrus.Infof("created dns service %q", service.Name)
	}
...
func (h *Handler) ensureDNSDeleted(dns *dnsv1alpha1.ClusterDNS) error {
	...
	logrus.Infof("deleted dns daemonset %q", ds.Name)
	return nil
}
